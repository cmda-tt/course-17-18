...
function render(error, data) {
	...
	chart.append("path").attr("d", arc).style("fill", fill), chart.append("text").attr("transform", transform).attr("dy", ".35em").text(age);
}

function type(chart) {
	return chart.population = Number(population(chart)), chart;
}

function transform(chart) {
	return "translate(" + arc.centroid(chart) + ")";
}

function fill(chart) {
	return color(age(chart));
}

function age(chart) {
	return chart.data.age;
}
